...
void search_maze_asta(VecVecint maze, bool is_pruning){
    ...
    //全ての探索した状態を保存する．
    std::multimap<point, maze_state> visited_maps;
    //探索候補を優先度付きキューに保存
    std::priority_queue<maze_state, std::vector<maze_state>,
        std::greater<maze_state>> que;

    maze_state mazestate(current, maze, point(0, 0), false);
    que.push(mazestate);
    visited_maps.insert(std::make_pair(current.pos, mazestate));
    ...
    while (!que.empty())
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            //同じ状態はスキップ
            if(is_exist_same_state(visited_maps, maze, next.pos) && is_pruning){
                continue;
            }
            ...
            if(state==PATH){
                ...
                que.push(mazestate);
                visited_maps.insert(std::make_pair(next.pos, mazestate));
            }
            ...
            if(state==GOAL){
                ...
                std::cout<< "min_step       : "<< next.step <<"\n";
                std::cout<< "total_step     : "<< visited_maps.size()  <<"\n";
                std::cout<< "state size     : " << visited_maps.size() <<"\n";
                clear_any(que);
                clear_any(visited_maps);
                break;
            }
        }
    }
...
bool search_maze_dfs(VecVecint maze,int cutoff,int cutoff_cost,int* sum_step, int *max_state_size, bool is_pruning){
    ...
    *sum_step+=step_cnt;
    *max_state_size = std::max(*max_state_size, (int)visited_maps.size());

    return find;
}

void search_maze_iidfs(VecVecint maze, bool is_pruning){
    //打ち切り時間計測用
    auto start_time = std::chrono::system_clock::now();
    //cutoff
    int cutoff = 1;
    ...
    //見つかるまで
    while (!find){
        find = search_maze_dfs(maze,cutoff,-1, &sum_step, &max_state_size, is_pruning);
        //少しずつ深くする
        ++cutoff;
        //10秒以上で強制打ち切り
...
void search_maze_idasta(VecVecint maze, bool is_pruning){
    //打ち切り時間計測用
    auto start_time = std::chrono::system_clock::now();
    //costについてのcutoff
    int cutoff_cost = 1;
    ...
    int max_state_size = 0;
    while(!find){
        find = search_maze_dfs(maze,-1,cutoff_cost, &sum_step, &max_state_size, is_pruning);
        //少しずつ深くする
        ++cutoff_cost;
        //10秒以上で強制打ち切り
        auto now_time = std::chrono::system_clock::now();
